...
Vagrant.configure("2") do |config|
  ...
  # Install Terraform
  config.vm.provision "shell", inline: "wget https://releases.hashicorp.com/terraform/0.11.7/terraform_0.11.7_linux_amd64.zip -O /tmp/terraform.zip"
  config.vm.provision "shell", inline: "cd /usr/local/bin; unzip /tmp/terraform.zip"
  config.vm.provision "shell", inline: "chmod +x /usr/local/bin/terraform"
  config.vm.provision "shell", inline: "sudo mkdir -p /srv/terraform"
  config.vm.synced_folder "terraform/", "/srv/terraform", owner: "root", group: "root"
